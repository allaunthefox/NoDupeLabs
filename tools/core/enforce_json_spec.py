#!/usr/bin/env python3
"""Enforce JSON specification compliance.

This script validates JSON files for spec compliance:
- Valid JSON syntax
- Key ordering
- Trailing whitespace

Usage:
    python tools/core/enforce_json_spec.py [--fix] [--check]
"""
...
def check_json_syntax(content: str) -> list[dict]:
...
def enforce_spec(content: str, fix: bool = True) -> tuple[list[dict], str]:
    """Enforce JSON spec."""
    errors = []
    errors.extend(check_json_syntax(content))
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Enforce JSON spec")
    ...
    files = list(set([f for f in files if not any(x in f.parts for x in [".venv", "venv", "__pycache__", "node_modules"])]))
    # Exclude htmlcov coverage JSON
    files = [f for f in files if "htmlcov" not in str(f)]
    ...
    if total_errors > 0:
        ...
        print("All JSON files comply with spec")
        return 0
