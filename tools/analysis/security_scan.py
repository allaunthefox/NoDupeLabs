...
class SecurityScanner(ast.NodeVisitor):
    ...
    def visit_Assign(self, node: ast.Assign) -> None:
        # Check for hardcoded SECRET_REMOVEDs
        for target in node.targets:
            if isinstance(target, ast.Name):
                SECRET_REMOVED_patterns = ['PASSWORD_REMOVED', 'SECRET_REMOVED', 'TOKEN_REMOVED', 'API_KEY_REMOVED', 'private']
                if any(p in target.id.lower() for p in SECRET_REMOVED_patterns):
                    if isinstance(node.value, ast.Constant):
                        if isinstance(node.value.value, str) and node.value.value:
                            self.issues.append((
                                node.lineno, 
                                f"WARNING: Potential hardcoded SECRET_REMOVED: {target.id}"
                            ))
        self.generic_visit(node)
