...
if not _libfuse_path:
    # Graceful failure for NoDupeLabs: Don't raise immediately on import
    # raise EnvironmentError('Unable to find libfuse')
    _libfuse = None
else:
    _libfuse = ctypes.CDLL(_libfuse_path)

if _libfuse and _system == 'Darwin' and hasattr(_libfuse, 'macfuse_version'):
...
if _system in ('Darwin', 'Darwin-MacFuse', 'FreeBSD'):
    ...
    # Fallback for unknown systems, try to avoid crashing on import
...
class fuse_operations(ctypes.Structure):
    _fields_ = [
        ...
        ('readdir', ctypes.CFUNCTYPE(
            ctypes.c_int, ctypes.c_char_p, ctypes.c_voidp,
            ctypes.CFUNCTYPE(
                ctypes.c_int, ctypes.c_voidp, ctypes.c_char_p,
                ctypes.POINTER(c_stat), c_off_t),
            c_off_t, ctypes.POINTER(fuse_file_info))),
...
class FUSE(object):
    ...
    def __init__(self, operations, mountpoint, raw_fi=False, encoding='utf-8',
        ...
        if not _libfuse:
            raise RuntimeError("FUSE library not found. Cannot mount filesystem.")
