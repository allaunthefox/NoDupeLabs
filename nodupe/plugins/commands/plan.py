...
class PlanPlugin(Plugin):
    ...
    def execute_plan(self, args: argparse.Namespace) -> int:
        ...
        try:
            ...
            groups = {}
            for f in files:
                if not f.get('hash'): continue
                if f['hash'] not in groups: groups[f['hash']] = []
            ...
            print(f"[PLUGIN] Summary: {stats['duplicates_found']} duplicates identified in {stats['total_groups']} groups.")
            if stats['reassigned'] > 0:
                print(f"[PLUGIN] Reassigned {stats['reassigned']} files as originals based on strategy.")
