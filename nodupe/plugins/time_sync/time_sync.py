...
class LeapYearCalculator:
    ...
    def _initialize_leap_year_plugin(self):
        ...
        try:
            ...
            self._leap_year_plugin = LeapYearPlugin(calendar="gregorian")
            self._leap_year_plugin.initialize()
            self._use_plugin = True
            logger.info("LeapYear plugin loaded successfully for time calculations")
    ...
    def is_leap_year(self, year: int) -> bool:
        ...
        """
        if self._use_plugin and self._leap_year_plugin:
            try:
                return self._leap_year_plugin.is_leap_year(year)
            except Exception as e:
                logger.warning(f"LeapYear plugin error, falling back to built-in: {e}")
...
class TimeSyncDisabledError(RuntimeError):
    """Raised when a requested operation is disabled on the TimeSync instance."""
    pass
...
class TimeSyncPlugin(Plugin):
    ...
    def initialize(self) -> None:
        ...
        logger.info("Initializing TimeSync plugin")
        if self._enabled:
    ...
    def shutdown(self) -> None:
        """Shutdown the plugin."""
        logger.info("Shutting down TimeSync plugin")
        self.stop_background(wait=False)

    # ---- abstract methods required by Plugin base class ----
    @property
    def name(self) -> str:
        """Plugin name."""
        return "TimeSync"
    ...
    def enable(self) -> None:
        """Enable the plugin."""
        self._enabled = True
        logger.info("TimeSync plugin enabled")

    def disable(self) -> None:
        """Disable the plugin."""
        self._enabled = False
        self.stop_background(wait=False)
        logger.info("TimeSync plugin disabled")
    ...
    def enable_network(self) -> None:
        ...
        logger.info("TimeSync network operations enabled")

    def disable_network(self) -> None:
        """Disable network operations."""
        self._allow_network = False
        logger.info("TimeSync network operations disabled")
    ...
    def enable_background(self) -> None:
        ...
        logger.info("TimeSync background synchronization enabled")

    def disable_background(self) -> None:
        """Disable background synchronization."""
        self._allow_background = False
        self.stop_background(wait=False)
        logger.info("TimeSync background synchronization disabled")
    ...
    def force_sync(self) -> Tuple[str, float, float, float]:
        ...
        Raises:
            TimeSyncDisabledError if the instance or network operation is disabled.
            RuntimeError on network / NTP failures.
        """
        if not self.is_enabled():
            raise TimeSyncDisabledError("TimeSync instance is disabled")
    ...
    def get_authenticated_time(self, format: str = "iso8601") -> str:
        ...
        Example usage:
            ...
            >>> plugin.get_authenticated_time(format="unix")
            "1702918204.123456"
            >>> plugin.get_authenticated_time(format="failure")
    ...
    def start_background(self, interval: float = 300.0, initial_sync: bool = True) -> None:
        ...
        Raises:
            TimeSyncDisabledError if disabled or background not allowed.
        """
        if not self.is_enabled():
            raise TimeSyncDisabledError("TimeSync instance is disabled")
        if not self.is_background_allowed():
            raise TimeSyncDisabledError("Background syncing is disabled for TimeSync")
        if not self.is_network_allowed():
            raise TimeSyncDisabledError("Cannot start background sync when network is disabled")
        ...
        self._bg_thread = threading.Thread(target=_loop, daemon=True, name="TimeSyncThread")
        self._bg_thread.start()
        logger.info(f"TimeSync background synchronization started (interval: {interval}s)")
    ...
    def stop_background(self, wait: bool = True, timeout: Optional[float] = None) -> None:
        ...
        logger.info("TimeSync background synchronization stopped")
    ...
    def fastdate64_to_iso(value: int) -> str:
        ...
        """
        ts = TimeSyncPlugin.decode_fastdate64(value)
    ...
    def iso_to_fastdate64(iso: str) -> int:
        ...
        ts = dt.timestamp()
        return TimeSyncPlugin.encode_fastdate64(ts)
    ...
    def get_days_in_february(self, year: int) -> int:
        ...
        Example:
            ...
            29
            >>> plugin.get_days_in_february(2023)
            28
        """
    ...
    def is_leap_year_plugin_available(self) -> bool:
        ...
        Example:
            ...
            True
        """
        return self._leap_year_calculator.is_plugin_available()
