"""Compatibility shim so tests and callers can import convert_videos from the package.

This tries to import the top-level `convert_videos` module (keeps existing CLI/utility
scripts working) and re-exports the `convert_video` helper so callers can import
`nodupe.convert_videos.convert_video` reliably whether the package is installed or
running from source.
"""
