...
class ArchiveHandler:
    ...
    def _detect_type(self) -> str:
        ...
        if s.endswith((".tar.gz", ".tgz")): return "tar"
        if s.endswith((".tar.bz2", ".tbz2")): return "tar"
        if s.endswith((".tar.xz", ".txz")): return "tar"
        return "unknown"

    def list_contents(self) -> List[Dict[str, Any]]:
        if self.type == "zip": return self._list_zip()
        if self.type == "7z": return self._list_7z()
        if self.type == "rar": return self._list_rar()
        if self.type == "tar": return self._list_tar()
        raise ValueError(f"Unsupported archive type: {self.type}")

    def extract(self, dest: Path) -> None:
        dest = Path(dest)
        dest.mkdir(parents=True, exist_ok=True)
        if self.type == "zip": self._extract_zip(dest)
        elif self.type == "7z": self._extract_7z(dest)
        elif self.type == "rar": self._extract_rar(dest)
        elif self.type == "tar": self._extract_tar(dest)
        else: raise ValueError(f"Unsupported archive type: {self.type}")
    ...
    def _list_7z(self):
        if not HAS_7Z: raise ImportError("py7zr not installed")
        results = []
        try:
    ...
    def _list_tar(self):
        ...
        with tarfile.open(self.path, 'r:*') as tf:
            for member in tf.getmembers():
                results.append({"path": member.name, "size": member.size, "is_dir": member.isdir()})
        return results
