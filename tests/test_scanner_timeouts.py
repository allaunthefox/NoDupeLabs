...
def test_threaded_hash_times_out_when_no_progress(tmp_path):
    ...
    def slow_process_file(p, algo, known_hash=None):
        # Block on an event so the future never completes within the test's
        # short timeouts
        block_event.wait(5)
        # return same shape as original
        st = os.stat(p)
        return (
            str(p), st.st_size, int(st.st_mtime), "deadbeef",
            None, None, algo, None
        )

    from nodupe.scan import hasher
    orig = hasher.process_file
    hasher.process_file = slow_process_file
    try:
        gen = scanner.threaded_hash(
            roots, ignore=[], workers=1, stall_timeout=0.02,
            max_idle_time=0.01, show_eta=False
        )
        with pytest.raises(TimeoutError):
            # consume generator - should raise due to max_idle_time
            list(gen)
    finally:
        hasher.process_file = orig
...
def test_threaded_hash_prints_stall_and_eta(tmp_path, capsys):
    ...
    def varied_process_file(p, algo, known_hash=None):
        ...
        # make the first several calls slow so the pending pool is fully
        # occupied
        if call_count["n"] <= 5:
    ...
    from nodupe.scan import hasher
    orig = hasher.process_file
    hasher.process_file = varied_process_file
    try:
        gen = scanner.threaded_hash(
            roots, ignore=[], workers=1, stall_timeout=0.05,
            max_idle_time=0.5, show_eta=True
        )
