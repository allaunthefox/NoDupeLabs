...
from nodupe.utils.filesystem import (
...
class TestUtils(unittest.TestCase):
    ...
    def test_hash_file(self):
        ...
        try:
            ...
            expected_sha512 = (
                "309ecc489c12d6eb4cc40f50c902f2b4d0ed77ee511a7c7a9bcd3ca86"
                "d4cd86f989dd35bc5ff499670da34255b45b0cfd830e81f605dcf7dc"
                "5542e93ae9cd76f"
            )
            self.assertEqual(
                hash_file(tmp_path, "sha512"), expected_sha512
            )
    ...
    def test_should_skip(self):
        ...
        self.assertTrue(
            should_skip(Path("/path/to/.git/config"), ignore_list)
        )
        self.assertTrue(
            should_skip(
                Path("/path/to/node_modules/package.json"), ignore_list
            )
        )
        self.assertFalse(
            should_skip(Path("/path/to/src/main.py"), ignore_list)
        )

    def test_detect_context(self):
        self.assertEqual(
            detect_context(Path("/home/user/Downloads/extracted/file.txt")),
            "archived"
        )
        self.assertEqual(
            detect_context(Path("/home/user/Documents/project/file.txt")),
            "unarchived"
        )
        self.assertEqual(
            detect_context(Path("/backup/2025/data.db")), "archived"
        )

    def test_get_mime_safe(self):
        self.assertEqual(get_mime_safe(Path("image.webp")), "image/webp")
        self.assertEqual(
            get_mime_safe(Path("document.pdf")), "application/pdf"
        )
        self.assertEqual(
            get_mime_safe(Path("unknown.xyz123")),
