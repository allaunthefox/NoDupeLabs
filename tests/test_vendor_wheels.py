...
def test_onnxruntime_wheel_vendored():
    ...
    wheels = [p for p in vendor_dir.iterdir() if p.suffix == '.whl' and 'onnxruntime' in p.name.lower()]
    assert wheels, 'expected at least one vendored onnxruntime wheel in nodupe/vendor/libs'
...
def test_minimum_vendored_wheels():
    ...
    present = {p.name.lower() for p in vendor_dir.iterdir() if p.suffix == '.whl'}
    missing = [n for n in expected if not any(n in name for name in present)]
    assert not missing, f'Missing vendored wheels for: {missing} (present: {sorted(present)})'
