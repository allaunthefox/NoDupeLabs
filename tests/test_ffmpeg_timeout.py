...
def test_run_ffmpeg_timeout_kills():
    ...
    ok = run_ffmpeg_with_progress(
        cmd, expected_duration=5.0, timeout=0.5, force_mode='quiet', probe_input_duration=False)
    assert ok is False
...
def test_run_ffmpeg_eta_and_success():
    ...
    ok = run_ffmpeg_with_progress(cmd, expected_duration=1.0, timeout=2.0,
                                  force_mode='quiet', probe_input_duration=False, show_eta=True)
    assert ok is True
