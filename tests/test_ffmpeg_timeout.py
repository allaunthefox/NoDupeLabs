...
def test_run_ffmpeg_timeout_kills():
    # Run a short python sleep that exceeds the timeout so the helper will
    # kill it
    cmd = [sys.executable, '-c', 'import time; time.sleep(3)']
    # expected_duration is larger than timeout -> we should hit timeout
    # and return False
    ok = run_ffmpeg_with_progress(
        ...
        timeout=0.5,
        force_mode='quiet',
        probe_input_duration=False
    )
    assert ok is False


def test_run_ffmpeg_eta_and_success():
    # Short sleep that will finish quickly; ETA should be harmless
