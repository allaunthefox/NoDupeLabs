...
class TestArchiveHandler:
    ...
    def test_is_archive_file_zip(self):
        ...
        with patch(
            "nodupe.core.mime_detection.MIMEDetection.detect_mime_type",
    ...
    def test_is_archive_file_tar(self):
        ...
        with patch(
            "nodupe.core.mime_detection.MIMEDetection.detect_mime_type",
            return_value="application/x-tar",
        ):
    ...
    def test_extract_nonexistent_archive(self):
        ...
        with pytest.raises(ArchiveHandlerError, match="Archive file not found"):
            self.archive_handler.extract_archive(
                str(nonexistent_path), str(extract_path)
            )
    ...
    def test_extract_invalid_archive(self):
        ...
        with patch(
            ...
            with pytest.raises(
                ArchiveHandlerError, match="Failed to extract archive"
            ):
                self.archive_handler.extract_archive(
                    str(invalid_path), str(extract_path)
                )
    ...
    def test_extract_archive_error_handling(self):
        ...
        with patch(
            ...
            with patch(
                "nodupe.core.compression.Compression.extract_archive",
    ...
    def test_get_archive_contents_info(self):
        ...
        with patch(
            ...
            contents_info = self.archive_handler.get_archive_contents_info(
                str(zip_path), str(base_path)
            )
    ...
    def test_extract_archive_multiple_formats(self):
        ...
        for archive_path, mime_type in test_files:
            with patch(
                ...
                extracted_files = self.archive_handler.extract_archive(
                    str(archive_path), str(extract_path)
                )
                assert len(extracted_files) == 1
    ...
    def test_extract_archive_performance(self):
        ...
        assert (
            elapsed < 10.0
        ), f"Extraction took too long: {elapsed:.2f} seconds"
    ...
    def test_extract_archive_memory_efficiency(self):
        ...
        for i in range(10):
            ...
            with zipfile.ZipFile(zip_path, "w") as zf:
                zf.writestr(
                    f"test_{i}.txt", f"content_{i}" * 1000
                )  # Larger content
    ...
    def test_extract_archive_corrupted_archive(self):
        ...
        with patch(
            ...
            with pytest.raises(ArchiveHandlerError):
                self.archive_handler.extract_archive(
                    str(corrupted_path), str(extract_path)
                )
    ...
    def test_extract_archive_nested_archives(self):
        ...
        with patch(
            ...
            extracted_files = self.archive_handler.extract_archive(
                str(outer_zip_path), str(extract_path)
            )
    ...
    def test_extract_archive_case_sensitivity(self):
        ...
        assert any(
            ...
            for name in ["Test.TXT", "test.txt", "TEST.Txt"]
        )
    ...
    def test_extract_archive_large_file_performance(self):
        ...
        assert (
            elapsed < 60.0
        ), f"Large file extraction took too long: {elapsed:.2f} seconds"
    ...
    def test_extract_archive_memory_usage_large_files(self):
        ...
        ) as zf:  # Use stored to avoid compression overhead
            zf.write(large_file, "large.txt")
    ...
    def test_extract_archive_concurrent_extraction(self):
        ...
        for zip_path in zip_files:
            thread = threading.Thread(
                target=extract_with_timing, args=(zip_path,)
            )
            threads.append(thread)
        ...
        individual_times = [
            end - start for start, end in zip(start_times, end_times)
        ]
    ...
    def test_extract_archive_error_recovery_partial(self):
        ...
        with patch(
            ...
            extracted_files1 = self.archive_handler.extract_archive(
                str(zip_path), str(extract_path)
            )
        ...
        with patch(
            ...
            extracted_files2 = self.archive_handler.extract_archive(
                str(zip_path), str(extract_path)
            )
        ...
        for i in range(1, 4):
            file1 = next(
                f for f in extracted_files1 if f.name == f"file{i}.txt"
            )
            file2 = next(
                f for f in extracted_files2 if f.name == f"file{i}.txt"
            )
    ...
    def test_extract_archive_archive_bomb_protection(self):
        ...
        if memory_used > 0:
            ...
            ), f"Memory usage too high: {memory_used / 1024 / 1024:.2f} MB"
    ...
    def test_extract_archive_mixed_content_types(self):
        ...
        assert (
            extracted_dict["binary.bin"].read_bytes()
        ...
        assert (
            extracted_dict["data.json"].read_text()
            == '{"key": "value", "number": 42}'
        )
    ...
    def test_extract_archive_nested_archive_extraction(self):
        ...
        with patch(
            ...
            outer_files = self.archive_handler.extract_archive(
                str(outer_zip_path), str(extract_path)
            )
        ...
        with patch(
            ...
            inner_files = self.archive_handler.extract_archive(
                str(inner_zip_extracted), str(inner_extract_path)
            )
    ...
    def test_extract_archive_temp_directory_isolation(self):
        ...
        assert not (
            Path(self.archive_handler._temp_dirs[0]) / "file2.txt"
        ).exists()

        assert (Path(self.archive_handler._temp_dirs[1]) / "file2.txt").exists()
        assert not (
            Path(self.archive_handler._temp_dirs[1]) / "file1.txt"
        ).exists()
    ...
    def test_extract_archive_error_message_details(self):
        ...
        assert (
            "BadZipFile" in error_message
            or "not a zip file" in error_message.lower()
        )
    ...
    def test_extract_archive_memory_leak_detection(self):
        ...
        for i in range(50):
            ...
            with zipfile.ZipFile(zip_path, "w") as zf:
                zf.writestr(
                    f"test_{i}.txt", f"content_{i}" * 100
                )  # Some content
        ...
        ), f"Memory usage too high: {total_memory / 1024 / 1024:.2f} MB"
    ...
    def test_extract_archive_file_handle_leak_detection(self):
        ...
        assert (
            handle_leak < 10
        ), f"File handle leak detected: {handle_leak} handles"
    ...
    def test_extract_archive_error_propagation(self):
        ...
        with patch(
            "nodupe.core.mime_detection.MIMEDetection.detect_mime_type",
            side_effect=Exception("MIME detection failed"),
        ):
            with pytest.raises(ArchiveHandlerError) as exc_info:
                self.archive_handler.extract_archive(
                    str(zip_path), str(extract_path)
    ...
    def test_extract_archive_cleanup_robustness(self):
        ...
        with patch(
            "shutil.rmtree",
            side_effect=[None, Exception("Cleanup failed"), None],
        ):
            # Should not raise exception
            self.archive_handler.cleanup()
    ...
    def test_extract_archive_concurrent_resource_usage(self):
        ...
        for zip_path in zip_files:
            thread = threading.Thread(
                target=extract_with_monitoring, args=(zip_path,)
            )
            threads.append(thread)
        ...
        ), f"Memory usage too high: {total_memory_used / 1024 / 1024:.2f} MB"
        assert (
            handle_growth < 50
        ), f"File handle growth too high: {handle_growth}"
    ...
    def test_extract_archive_error_recovery_comprehensive(self):
            ...
            with pytest.raises(scenario["expected_error"]) as exc_info:
                if scenario["name"] == "nonexistent_file":
                    ...
                    with patch(
                        ...
                        self.archive_handler.extract_archive(
                            str(zip_path), str(readonly_path)
                        )
        ...
        with patch(
            ...
            extracted_files = self.archive_handler.extract_archive(
                str(valid_zip), str(extract_path)
            )
