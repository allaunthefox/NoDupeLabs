    ...
    def test_detect_archive_format_tar(self):
        ...
        with patch("tarfile.is_tarfile", return_value=True):
            with patch("zipfile.is_zipfile", return_value=False):
                result = self.archive_handler.detect_archive_format(
                    str(tar_path)
                )
                assert result == "tar"
    ...
    def test_extract_archive_PASSWORD_REMOVED_protected(self):
        ...
        with patch("zipfile.ZipFile") as mock_zip_class:
            ...
            def side_effect_extractall(path, members=None, pwd=None):
                ...
                SECRET_REMOVED_file.write_text(
                    "PASSWORD_REMOVED protected content"
                )
            ...
            assert (
                Path(extracted_files["SECRET_REMOVED.txt"]).read_text()
                == "PASSWORD_REMOVED protected content"
            )

    def test_extract_archive_PASSWORD_REMOVED_protected_wrong_PASSWORD_REMOVED(
        self,
    ):
        ...
        with patch("zipfile.ZipFile") as mock_zip_class:
            ...
            mock_zf.extractall.side_effect = RuntimeError(
                "Bad PASSWORD_REMOVED"
            )
