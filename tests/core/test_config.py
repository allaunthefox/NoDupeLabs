...
class TestConfigManager:
    ...
    def test_config_manager_missing_nodupe_section(self):
        ...
        config_data = {"tool": {"other_tool": {"some_config": "value"}}}

        with open(self.config_path, "w") as f:
            toml.dump(config_data, f)

        with pytest.raises(
            ValueError, match="missing \\[tool.nodupe\\] section"
        ):
            ConfigManager(str(self.config_path))
    ...
    def test_get_scan_config(self):
        ...
        assert scan_config["default_extensions"] == [
            "jpg",
            "png",
            "pdf",
            "docx",
    ...
    def test_get_config_value(self):
        ...
        nonexistent = config_manager.get_config_value(
        ...
        config_data = {
            "tool": {
                "nodupe": {
                    ...
                    "performance": {"parallel": True, "cache_enabled": False},
                }
            }
        }
            ...
            "tool": {
                "nodupe": {
                    "scan": {
                        ...
                        "include_patterns": [
                            f"pattern_{i}" for i in range(500)
                        ],
                    },
                    "database": {
                        "paths": [f"/path_{i}" for i in range(100)],
                        "connection_strings": {
                            f"conn_{i}": f"sqlite:///{i}.db" for i in range(50)
                        },
                    },
                    "performance": {
                        "worker_settings": {
                            f"worker_{i}": i for i in range(100)
                        },
