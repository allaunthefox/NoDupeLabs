...
class TestConfigManager:
    ...
    def test_init_with_default_path(self, mock_exists: MagicMock, mock_open: MagicMock):
        ...
        if os.path.exists("pyproject.toml"):
             # If it exists (e.g. running in real env), it might work.
             # But we're mocking exists=False
    ...
    def test_get_nodupe_config(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.toml', delete=False) as f:
            ...
            f.flush()
            f.close()  # Close file so it can be opened by ConfigManager on Windows
...
# Missing logging section
    ...
    def test_config_manager_file_not_found(self):
        """Test ConfigManager raises FileNotFoundError for non-existent file"""
        with pytest.raises(FileNotFoundError):
            ConfigManager("non_existent_file.toml")

    def test_config_manager_invalid_toml(self):
        """Test ConfigManager raises ValueError for invalid TOML content"""
        with tempfile.NamedTemporaryFile(mode='w', suffix='.toml', delete=False) as f:
