...
Removed explicit `CODECOV_TOKEN` reference in workflow. The Codecov action now works without requiring a token for public repositories.
...
- The `PYPI_API_TOKEN` secret is still required for publishing to PyPI
- The `GITHUB_TOKEN` is automatically provided by GitHub Actions
- Codecov integration now works without explicit token for public repositories
