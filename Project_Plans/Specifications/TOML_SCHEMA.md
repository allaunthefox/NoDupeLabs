...
### TOML Standard (ISO/IEC 21646:2023)

**Standard**: ISO/IEC 21646:2023 - Information technology — TOML (Tom's Obvious, Minimal Language)
**Official Name**: TOML
**Status**: International Standard published by ISO/IEC
**Publication Date**: 2023
**Reference**: https://www.iso.org/standard/82444.html

### JSON Schema Standard

**Standard**: JSON Schema Draft 7 (IETF RFC 8259)
**Format**: TOML → JSON/JSON-L conversion for validation
**Validation Tools**: ajv, jsonschema, python-jsonschema, etc.

### Format Standards

**JSON**: RFC 8259 (The JavaScript Object Notation (JSON) Data Interchange Format)
**JSON-L**: RFC 7464 (JSON Text Sequences)
...
2. **Validation Vocabulary**: `minimum`, `maximum`, `pattern`, `enum`
3. **Metadata Vocabulary**: `title`, `description`, `default`
4. **Structure**: `$ref` for references, `definitions` for reusable schemas

### Industry Standards Referenced

1. **Semantic Versioning**: `version` field follows SemVer 2.0.0
2. **Syslog Levels**: `log_level` follows RFC 5424
3. **FIPS 180-4**: Hash algorithms follow NIST standards
4. **SQLite Standards**: Database parameters follow SQLite conventions
5. **Glob Patterns**: File patterns follow POSIX glob standards
...
2. **ISO 15836-1:2017** - Information and documentation — The Dublin Core metadata element set
   - Dublin Core Metadata Initiative (DCMI) standard
   - 15 core metadata elements for resource description

3. **ISO 19139:2007** - Geographic information — Metadata — XML schema implementation
   - XML implementation of ISO 19115 for geospatial metadata

4. **ISO 23081-1:2017** - Information and documentation — Records management processes — Metadata for records
...
2. **Validation Tools**: Use standard-compliant validation tools
3. **Conversion Utilities**: Provide conversion between TOML/JSON and standard metadata formats
4. **Documentation**: Document metadata standard compliance in specifications
...
### JSON Standard Support (RFC 8259)

**Standard**: JSON (JavaScript Object Notation) - RFC 8259
...
### JSON-L (JSON Lines) Standard Support (RFC 7464)

**Standard**: JSON Lines - RFC 7464
**MIME Type**: `application/json-lines`
...
{"database": {"pool_size": 5, "synchronous": "NORMAL", "cache_size": -2000}}
```
...
1. **TOML Tables** → **JSON Objects**
2. **TOML Arrays** → **JSON Arrays**
3. **TOML Primitives** → **JSON Primitives** (strings, numbers, booleans)
4. **TOML Dates** → **JSON Strings** (ISO 8601 format)

#### TOML to JSON-L Conversion Rules

1. **Each TOML Table** → **Separate JSON Line**
2. **Arrays and Primitives** → **Same as JSON conversion**
3. **Nested Tables** → **Separate JSON Lines with dot notation or nested objects**
4. **Comments** → **Removed (JSON-L doesn't support comments)**
...
2. **JSON-L**: Line-by-line validation or combined validation after parsing
...
2. **Backward Compatibility**: Major versions maintain backward compatibility
3. **Deprecation**: Deprecated properties supported for 2 major versions
4. **Validation**: Strict validation with clear error messages
...
2. **JavaScript**: `ajv` (npm)
3. **Java**: `networknt/json-schema-validator` (Maven)
4. **Go**: `xeipuuv/gojsonschema` (GitHub)
5. **Rust**: `jsonschema` (crates.io)
