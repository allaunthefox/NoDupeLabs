...
**Compatibility**: Backward compatible with version `0.9.x`
...
| `updated_at` | INTEGER | ❌ | Update timestamp | ≥ 0 |

**Status Values**: `active`, `deleted`, `archived`, `error`
...
**Relationship Types**: `duplicate`, `similar`, `related`, `version`
...
**Plugin Types**: `command`, `similarity`, `ml`, `gpu`, `video`, `network`
**Status Values**: `loaded`, `failed`, `disabled`, `unloaded`
...
**Data Types**: `string`, `integer`, `float`, `boolean`, `json`
...
| `expires_at` | INTEGER | ❌ | Expiration time | ≥ 0 |

**Algorithm Values**: `md5`, `sha1`, `sha256`, `sha512`
...
**Query Types**: `find_duplicates`, `find_similar`, `get_file_info`, `search_files`
...
| `data_type` | TEXT | ❌ | Data type | - |
| `created_at` | INTEGER | ❌ | Creation time | ≥ 0 |
| `updated_at` | INTEGER | ❌ | Update time | ≥ 0 |
...
2. **Set Null**: Used for optional relationships where null is acceptable
3. **Restrict**: Used for critical relationships that should prevent deletion

### Indexing Strategy

1. **Primary Keys**: Always indexed
2. **Foreign Keys**: Always indexed
3. **Unique Constraints**: Always indexed
4. **Frequently Queried Fields**: Indexed based on query patterns
5. **Sort/Filter Fields**: Indexed for performance

### Data Integrity Rules

1. **NOT NULL Constraints**: Enforced for required fields
2. **UNIQUE Constraints**: Enforced for unique identifiers
3. **CHECK Constraints**: Enforced for value ranges
4. **FOREIGN KEY Constraints**: Enforced for relationships
5. **DEFAULT Values**: Provided for optional fields
...
2. **Validation**: Validate current schema version
3. **Application**: Apply migration scripts in order
4. **Verification**: Verify schema changes
...
2. **VACUUM**: Regular database vacuuming
3. **ANALYZE**: Database analysis for query planning
...
2. **Migration Path**: Provide clear migration path for existing data
3. **Deprecation**: Deprecated tables/columns supported for 2 major versions
4. **Validation**: Schema validation on startup
5. **Documentation**: All schema changes must be documented
